...
describe("PUT /api/v1/user/terms", () => {
    it('#userterms 1 should fail when authorization is missing with status: 401', (done) => {
        console.log("Here");
        chai
        .request(server)
        .put("/api/v1/user/terms")
        .send(requestBody)
        .end((err, res) => {
            console.log("res",res)
    ...
    it("#userterms 2 Mandatory Fields is empty should return an status: 422 Ok.", (done) => {
        console.log("Here");
        let body={ }
        chai
          .request(server)
          ...
          .end((err, res) => {
            ...
            res.body.should.be.a("object");
            res.body.message.should.be.eql("termsAgreed is required");
            res.body.status.should.be.eql(422);
            done();
          });
      });

      it("#userterms 3 termsAgreed pass Empty value should return an status: 422 Ok.", (done) => {
        let body={
            "termsAgreed": ""
         }
        chai
        ...
        .end((err, res) => {
          res.should.have.status(422);
          res.body.should.be.a("object");
          res.body.message.should.be.eql("termsAgreed must be a boolean");
          res.body.status.should.be.eql(422);
          done();
        });
    });
    it("#userterms 4 User not Found should return an status: 409 Ok.", (done) => {
        console.log("Here");
        chai
          .request(server)
          .put("/api/v1/user/terms")
